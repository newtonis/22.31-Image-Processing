...
downsampled_dani = img_original[0::4,0::4]
img_rgb = cv2.cvtColor(downsampled_dani,cv2.COLOR_GRAY2RGB)
...
dst = bicubic(img_rgb, ratio, a)
print('Completed!')
# img_gray = cv2.cvtColor(dst,cv2.COLOR_RGB2GRAY)
#  cv2.imwrite('bicubic_butterfly.png', dst)

fig1, axs1 = plt.subplots(nrows=1, ncols=2)
axs1[0].imshow(downsampled_dani,cmap="gray", vmin=0, vmax=255)
axs1[1].imshow(dst.astype('uint8'),cmap="gray")
